...
ALTER TABLE user_info DROP COLUMN user_phone;
ALTER TABLE user_info ADD (pwd varchar2(100)); 
ALTER TABLE user_info RENAME COLUMN pwd TO user_pwd; 
alter table user_info modify (user_email varchar2(200));
alter table user_info modify (user_id varchar2(100)) ;
alter table user_info modify (user_nickname varchar2(100)) ;
alter table user_info modify (user_pwd varchar2(100)) ;
desc user_info;
--카테고리분류메뉴
create table cate (
    c_num               NUMBER,                 --pk  key
...
create table board_like (
    b_like_num number REFERENCES board(b_num),
    b_like_id varchar2(100) REFERENCES user_info(user_id)
);
--업로드 파일
